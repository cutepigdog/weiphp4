        ...
        <div class="table-striped">
          <table cellspacing="1">
            ...
            <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                ...
                <?php if(($data["status"]) == "已开启"): ?><a href="javascript:void(0)" onclick="setStatus(<?php echo ($data["id"]); ?>, 0)">禁用</a>&nbsp;&nbsp;&nbsp;
                <?php else: ?>
                <a href="javascript:void(0)" onclick="setStatus(<?php echo ($data["id"]); ?>, 1)">启用</a>&nbsp;&nbsp;&nbsp;<?php endif; ?>
                
                <a href="<?php echo U('edit',array('id'=>$data['id']));?>" target="_self">编辑</a>&nbsp;&nbsp;&nbsp;
                <a href="<?php echo addons_url('Reserve://ReserveValue/lists',array('reserve_id'=>$data['id']));?>" >预约列表</a>&nbsp;&nbsp;&nbsp;
                <a href="<?php echo U('del',array('id'=>$data['id']));?>" class="confirm">删除</a>&nbsp;&nbsp;&nbsp;
                <a data-clipboard-text="<?php echo addons_url('Reserve://Wap/index',array('token'=>$token,'reserve_id'=>$data['id'],'publicid'=>$public_info['id'],));?>" class="list_copy_link" id="copyLink_<?php echo ($data["id"]); ?>">复制链接</a></td>
