...
<table class="">
	...
	<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            ...
            <td><?php echo ($vo["addon_title"]); ?></td>
			<td>
            <?php if(!empty($vo["table_exists"])): if(!empty($vo["update_file"])): ?><a href="<?php echo U('freshDBtoFile?model_id='.$vo['id']);?>">更新文件</a>&nbsp;&nbsp;&nbsp;<?php endif; ?>
            <?php if(!empty($vo["update_db"])): ?><a href="<?php echo U('freshFiletoDB?model_id='.$vo['id']);?>">更新数据表</a>&nbsp;&nbsp;&nbsp;<?php endif; ?>
            
            <a href="<?php echo U('attribute/index?model_id='.$vo['id']);?>">字段管理</a>&nbsp;&nbsp;&nbsp;
			<a href="<?php echo U('model/edit?id='.$vo['id']);?>#2">编辑</a>               
                <?php else: ?>
<a href="<?php echo U('importModel',['model_name'=>$vo['name'],'model_addon'=>$vo['addon']]);?>">导入模型</a><?php endif; ?>
