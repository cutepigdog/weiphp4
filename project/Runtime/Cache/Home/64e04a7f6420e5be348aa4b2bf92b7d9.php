            ...
            <table cellspacing="1">
              ...
              <tbody>
                <?php if(is_array($list_data[0])): $i = 0; $__LIST__ = $list_data[0];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <a href="<?php echo U('Home/Apps/check_res',['wpid'=>$data['id'],'id'=>$data['id']]);?>" target="_self">自动检测</a>&nbsp;&nbsp;&nbsp; 
                    <?php if($data['is_creator']) { ?>
                    <a href="<?php echo U('Home/AppsLink/lists',['wpid'=>$data['id'],'mp_id'=>$data['id']]);?>" target="_self">管理员配置</a>&nbsp;&nbsp;&nbsp;
                      <?php if(!C('PUBLIC_BIND') || !$data['is_bind']) { ?>
                      <a href="<?php echo U('step_0',['wpid'=>$data['id'],'id'=>$data['id']]);?>" target="_self">编辑</a>&nbsp;&nbsp;&nbsp;
                      <a href="<?php echo U('payment_set',['wpid'=>$data['id'],'id'=>$data['id']]);?>" target="_self">微信支付</a>&nbsp;&nbsp;&nbsp;
                      <?php } } ?>
