...
#phone {
	...
	width: 510px;
	height: 814px;
	background: url(/Public/Home/images/preview_phone.jpg) no-repeat;
}
#frame {
        ...
        <label class="item-label"> 自定义背景图: <span class="check-tips">为空时不显示背景图片，最佳尺寸：640X1008,或上传比例为640X1008的更大尺寸图片</span> </label>
			 ...
			 <div class="mult_imgs">
                                <div class="upload-img-view" id='mutl_picture_background'>
                                  <?php if(!empty($data[background_arr])): if(is_array($data[background_arr])): $i = 0; $__LIST__ = $data[background_arr];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(!empty($vo)): ?><div class="upload-pre-item22">
