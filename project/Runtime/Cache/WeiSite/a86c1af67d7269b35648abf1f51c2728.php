...
<link href="<?php echo ADDON_PUBLIC_PATH;?>/weisite.css?v=<?php echo SITE_VERSION;?>" rel="stylesheet" type="text/css">
  <div class="span9 page_message">
    <section id="contents"> 
       <div class="setting_step app_setting content_step content_step_2" style="margin:20px;">
         <a class="step step_cur" href="<?php echo addons_url('WeiSite://Template/footer',array('mdm'=>I('mdm')));?>">1.选择模板</a>
       	 <a class="step step_cur_1" href="<?php echo addons_url('WeiSite://Footer/lists',array('mdm'=>I('mdm')));?>">2.导航配置</a>
         <!--<a class="step " href="<?php echo addons_url('WeiSite://WeiSite/preview',array('mdm'=>I('mdm')));?>">3.效果预览</a>-->
  </div>
  
            <?php if(!$has_data): ?><p style="padding:80px 30px 30px; text-align:center; font-size:18px">
      您选择了无导航栏模板，不需要配置导航了！
      </p>
       <!--<p style="padding:30px; text-align:center"><a class="btn submit-btn" href="<?php echo addons_url('WeiSite://WeiSite/preview');?>">预览</a></p>-->
      <?php else: ?>
      ...
      <div class="table-bar">
        ...
        <?php if($search_button): ?><div class="search-form fr cf">
          <div class="sleft">
            <?php $search_url || $search_url = addons_url(MODULE_NAME.'://'.CONTROLLER_NAME.'/lists',array('model'=>$model['name'])); ?>
      ...
      <div class="data-table">
        <div class="table-striped">
          <table cellpadding="0" cellspacing="1">
            ...
            <thead>
              <tr>
                <?php if($check_all): ?><th class="row-selected row-selected"> <input class="regular-checkbox check-all" id="checkAll" type="checkbox"><label for="checkAll"></label></th><?php endif; ?>
                <?php if(is_array($list_grids)): $i = 0; $__LIST__ = $list_grids;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;?><th <?php if(!empty($field["width"])): ?>style="width:<?php echo ($field["width"]); ?>px"<?php endif; ?> ><?php echo ($field["title"]); ?></th><?php endforeach; endif; else: echo "" ;endif; ?>
              </tr>
            </thead>
            ...
            <tbody>
              <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                  <?php if($check_all): ?><td><input class="regular-checkbox ids" type="checkbox" value="<?php echo ($data['id']); ?>" id="ids_<?php echo ($data['id']); ?>" name="ids[]"><label for="ids_<?php echo ($data['id']); ?>"></label></td><?php endif; ?>
      ...
      <div class="page"> <?php echo ((isset($_page) && ($_page !== ""))?($_page):''); ?> </div><?php endif; ?>
    </section>
  </div>
