...
//			// dump ( $html );
	...
	function lists() {
		...
		if (file_exists ( ONETHINK_ADDON_PATH . 'WeixinArticle/View/pigcms/Index_' . $this->config ['template_lists'] . '.html' )) {
			
			$this->pigcms_lists ( $cate_id );
			$this->display ( ONETHINK_ADDON_PATH . 'WeixinArticle/View/pigcms/Index_' . $this->config ['template_lists'] . '.html' );
		} else {
			$map ['token'] = get_token ();
			if ($cate_id) {
				$map ['cate_id'] = $cate_id;
			...
			if (! empty ( $category )) {
				foreach ( $category as &$vo ) {
					$vo ['icon'] = get_cover_url ( $vo ['icon'] );
					empty ( $vo ['url'] ) && $vo ['url'] = addons_url ( 'WeiSite://Wap/lists', array (
							'cate_id' => $vo ['id'] 
					) );
				}
				$this->assign ( 'category', $category );
				...
				if (empty ( $cate ['template'] )) {
					if ($htmlstr == 'cate.html')
						$cate ['template'] = 'default';
					else
						$cate ['template'] = 'ColorV1';
				}
				
				$this->display ( ONETHINK_ADDON_PATH . 'WeixinArticle/View/TemplateSubcate/' . $cate ['template'] . '/' . $htmlstr );
			} else {
				...
				$html = isset ( $this->config ['template_lists'] ) && ! empty ( $this->config ['template_lists'] ) ? $this->config ['template_lists'] : 'V1';
				$this->display ( ONETHINK_ADDON_PATH . 'WeixinArticle/View/TemplateLists/' . $html . '/lists.html' );
			}
		}
	}
	// 详情
	function detail() {
			...
			$info = M ( 'weixin_article' )->where ( $map )->find ();
			$this->assign ( 'info', $info );
			//M ( 'custom_reply_news' )->where ( $map )->setInc ( 'view_count' );
            $templateFile = ONETHINK_ADDON_PATH . 'WeixinArticle/View/TemplateDetail/V1/detail.html';
	...
	function _footer($temp_type = 'weiphp') {
		if ($temp_type == 'pigcms') {
			...
			$list = D ( 'Addons://WeixinArticle/Footer' )->get_list ();
			...
			$html = $this->fetch ( ONETHINK_ADDON_PATH . 'WeixinArticle/View/TemplateFooter/' . $this->config ['template_footer'] . '/footer.html' );
	 ...
	 * 5、把pigcms 目录下所有文档代码里的 Wap/Index/lists 替换成 WeixinArticle/Wap/lists
	 * 6、把pigcms 目录下所有文档代码里的 Wap/Index/index 替换成 WeixinArticle/Wap/index
	 */
	function pigcms_init() {
		...
		define ( 'RES', ONETHINK_ADDON_PATH . 'WeixinArticle/View/pigcms/common' );
		...
		// 模板信息
		if (file_exists ( ONETHINK_ADDON_PATH . MODULE_NAME . '/View/pigcms/index.Tpl.php' )) {
			$pigcms_temps = require_once ONETHINK_ADDON_PATH . MODULE_NAME . '/View/pigcms/index.Tpl.php';
			foreach ( $pigcms_temps as $k => $vo ) {
				$temps [$vo ['tpltypename']] = $vo;
			}
		}
		
		if (file_exists ( ONETHINK_ADDON_PATH . MODULE_NAME . '/View/pigcms/cont.Tpl.php' )) {
			$pigcms_temps = require_once ONETHINK_ADDON_PATH . MODULE_NAME . '/View/pigcms/cont.Tpl.php';
			foreach ( $pigcms_temps as $k => $vo ) {
				$temps [$vo ['tpltypename']] = $vo;
			}
		}
	...
	function _pigcms_cate($pid = null) {
		...
		foreach ( $category as $k => $vo ) {
			$param ['cate_id'] = $vo ['id'];
			$url = empty ( $vo ['url'] ) ? $vo ['url'] = addons_url ( 'WeiSite://Wap/lists', $param ) : $vo ['url'];
			$pid = intval ( $vo ['pid'] );
