...
function getOpenidByUid($uid, $token = '') {
	...
	$map ['uid'] = $uid;
	$map ['token'] = $token;
	return M ( 'apps_follow' )->where ( $map )->getField ( 'openid' );
}
/*
 * 获取支付的appid的openid
 * 微信支付和红包使用
...
function get_token($token = NULL) {
	$index = strtolower ( MODULE_NAME . '/' . CONTROLLER_NAME );
	if ($index == 'home/weixin') {
		if ($token !== NULL) {
			session ( 'token', $token );
		} else {
			$token = session ( 'token' );
...
function get_app_info($token = '', $field = '') {
	...
	if ($token != 'gh_3c884a361561') {
		$info = D ( 'Common/Apps' )->getInfoByToken ( $token, $field );
	}
	return $info;
}
// 兼容旧方法
function get_token_appinfo($token = '', $field = '') {
	return get_app_info ( $token, $field );
}
// 兼容旧方法
function get_service_info($field = '') {
	return get_app_info ( '', $field );
}
// 判断公众号的类型：是订阅号还是服务号
...
// 以POST方式提交数据
function post_data($url, $param, $type = 'json', $return_array = true, $useCert = [], $timeOut = 30) {
...
function add_admin_log($content, $mod = 'admin') {
	...
	$log ['content'] = is_array ( $content ) ? var_export ( $content, true ) : $content;
	if (strlen ( $log ['content'] >= 65535 )) {
		$log ['content'] = substr ( $log ['content'], 0, 65530 );
	}
	$log ['mod'] = $mod;
	$log ['cTime'] = NOW_TIME;
	M ( 'admin_log' )->add ( $log );
...
function add_request_log($url, $param = [], $res = [], $error_code = '', $msg = '') {
	...
	$log ['url'] = $url;
	$log ['param'] = is_array ( $param ) ? var_export ( $param, true ) : $param;
	...
	M ( 'request_log' )->where ( $map )->delete ();
}
...
function upload_media($path, $type = 'image') {
	...
	if (isset ( $res ['errcode'] ) && $res ['errcode'] != 0) {
		$this->error ( '120000:' . error_msg ( $res, '图片上传' ) );
		exit ();
	}
	return $res ['media_id'];
}
...
function addAutoCheckLog($na = '', $msg = '', $token = '') {
	...
	// dump ( $data );
	$res = M ( 'apps_check' )->add ( $data );
	// dump ( M ( 'apps_check' )->getLastSql() );
}
	...
	if (preg_match ( '#^/Uploads/Picture/#i', $imgurl )) { // 内部图片
		...
		if ($imginfo) { // 规格图片存在
			if ($imginfo [0] > $w || $imginfo ['1'] > $h) {
				...
				return str_replace ( './Uploads', SITE_URL . '/Uploads', $imgurl );
			}
		}
		return str_replace ( './Uploads', SITE_URL . '/Uploads', $dirname_new );
	}
